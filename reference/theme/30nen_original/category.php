<?php get_header(); ?>
                ...
                <section class="module02">
                    ...
                    <div class="box">
                        <?php
                    $category = get_queried_object(); // 現在表示中のカテゴリ情報を取得
                    ...
                    // 読み仮名（ふりがな）も任意で設定
                    $category_kana = get_term_meta($category->term_id, 'category_kana', true);
                    ?>

                    <div class="common_title03 flex">
                        <div class="margin"><img src="<?php echo $category_image; ?>" alt="<?php echo esc_attr($category->name); ?>"></div>
                        
                        <p>
                            <?php echo esc_html($category->name); ?><br>
                            <?php if (!empty($category_kana)) : ?>
                                <span class="letter_sp"><?php echo esc_html($category_kana); ?></span>
                            <?php endif; ?>
                        </p>
                    </div>
...
<?php if (!empty($child_categories)) : ?>
    <!-- 子カテゴリーがある場合：子カテゴリー画像を表示 -->
    <ul id="post-container" class="list_inner flex mt-10">
        <?php foreach ($child_categories as $child) :
            $child_image = get_term_meta($child->term_id, 'category_image', true);
            $child_image = !empty($child_image) ? esc_url($child_image) : get_theme_file_uri('common/jpg/line-up.png');
            $child_link  = get_category_link($child->term_id);
        ?>
            <li>
                <a href="<?php echo esc_url($child_link); ?>">
                    <img src="<?php echo $child_image; ?>" alt="<?php echo esc_attr($child->name); ?>">
                    <p class="writer"><?php echo esc_html($child->name); ?></p>
                </a>
            </li>
        <?php endforeach; ?>
    </ul>
...
<?php endif; ?>
                    ...
                    <div class="box writer mt-40">
                        ...
                        <div class="lineup_profile mb-40 flex">
                            ...
                            $user_ids = (array) get_term_meta($term->term_id, 'cat_user_ids', true);
                            ...
                            if (!empty($user_ids)) :
                                ...
                                <div class="w_100 flex">
                                    <figure>
                                    <img src="<?php echo $avatar_url; ?>" alt="<?php echo esc_attr($author_name); ?>">
                                    </figure>
                                    <div class="profile_text">
                                        <h2><?php echo esc_html($author_name); ?></h2>
                                        <p class="text_01">
                                        <?php if ($birthplace) echo esc_html($birthplace).'／'; ?>
