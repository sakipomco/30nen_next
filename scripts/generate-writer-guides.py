...
def build_ja():
    ...
    story.append(note_box(
        f"{SWITCH_DATE_JA} までは練習期間です。新システムで書いた内容はプレビューできますが、30nen.com の本番サイトには反映されません。切替日以降は、新しい投稿画面から投稿します。",
        s,
    ))
    story.append(Spacer(1, 5*mm))
    ...
    story.append(blist([
        ...
        "新規投稿で試し書きをして、画像アップロード、下書き保存、プレビューを試す。",
        "うまくいかないところはメッセンジャーグループ／グループLINEで知らせる。",
    ], s["bullet"]))

    story.append(p("ログインについて", s["h1"]))
    story.append(p("店主がアカウントを作ると、登録メールアドレス宛に<b>招待メール</b>が届きます。メール内のリンクを踏んで、自分でパスワードを設定してください。", s["body"]))
    story.append(blist([
