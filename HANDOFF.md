...
最終更新: 2026-06-12（固定ページ3本を新設＝「当店について」`/about`（項目37）・「サイトのご利用について」`/howtouse`（項目38）・「プライバシーポリシー」`/privacy`（項目39）。あわせて本文14px・足跡10pxに統一。左端メニューの残りは「沿革」のみ。切替時期は7月下旬 or 8月末を検討中）
...
39. **フェーズ2：「プライバシーポリシー」ページ `/privacy` を新設（SAKIさんと対話しながら文章も更新・ブラウザ動作確認済み・SAKIさんOK済み）**（コミット `（このコミット）`）。型チェック(tsc)・Lint(eslint)クリーン。左端メニューは残り「沿革」のみ。
    ...
    - **追い調整（同日・SAKIさんOK済み）**（コミット `（このコミット）`）: ①PC（lg以上）の**のれんロゴを約80%に縮小**（`sidebar-left.tsx`・`max-w-[260px]`→`lg:max-w-[208px]`。上端ぴったりの引き上げ `-mt-12` はページ上部の余白ぶんなので変えない＝縮小後も上端ずれ0pxを計測確認）。スマホのロゴは大きいまま。②**左端の縦書きメニューの文字を12pxに**（`edge-nav.tsx`・`text-sm`(14px)→`text-xs`）。
