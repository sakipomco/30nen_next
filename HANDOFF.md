...
13. 画像アップロード（本文への画像挿入＋アイキャッチ画像） … ✅ 完了（`src/app/api/upload/route.ts`・`featured-image.tsx`・DBに`featured_image_path`列追加・ブラウザ動作確認済み）
    ...
    - **やることメモ（未実装）：アイキャッチ画像 未設定の確認アラート**。投稿者が**アイキャッチ画像を設定せずに「投稿する」（公開）を押したとき**、確認メッセージを出す。
      - 方針＝**A案（やさしい）**：「アイキャッチ画像が未設定です。このまま公開しますか？」→「はい／戻る」を選べる。**画像なしでも公開はできる**（必須にはしない）。
      - 想定の作り方：投稿フォーム側（`src/app/admin/article-form.tsx` または `featured-image.tsx`）で、`intent=publish` のボタン押下時にアイキャッチ未設定なら `window.confirm(...)` で確認。キャンセルなら送信中止。下書き保存のときは出さない。
      - SAKIさんの希望（2026-06-02）。「20名の書き手のうっかり忘れ」を防ぐのが目的。
