...
最終更新: 2026-06-15（メールアイコンの行き先を手当て＝専用ページは作らず左コラムの「お便りフォーム」へ誘導＋スマホではお便りフォームを非表示に（項目41）。直前の固定ページ4本＝「当店について」`/about`（項目37）・「サイトのご利用について」`/howtouse`（項目38）・「プライバシーポリシー」`/privacy`（項目39）・「沿革」`/history`（項目40）で**左端メニュー5項目がすべて完成**。切替時期は7月下旬 or 8月末を検討中）
