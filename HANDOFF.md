...
最終更新: 2026-06-03（公開トップページのデザイン要件を A〜J 全項目確定・§14に記録）
...
20. **フェーズ2：公開トップページのデザイン要件を1項目ずつ確定**（コミット `c8f19d1`・`637f69f`）。
    ...
    - **先取りメモ**: スマホのハンバーガー開くアイコンは **`30nen_sanmaru.png`**（黒丸に三本線の三丸アイコン・現在 `ph_data/` にある→実装時に `public/` へ）。
    - **このセッションでは実装はしない方針**。前回着手しかけた実装ファイル（`src/components/` の部品・`datetime.ts` の追記・コピー画像）は消して、要件定義だけの状態に戻した。
    - **未確定（次回詰める）**: スマホ表示の詳細（左端メニュー・右端アイコン・右コラムの畳み方／最新エリアのスマホ表示／開閉アニメ）。`docs/public-page-spec.md` §6 をたたき台に確定する。
