...
最終更新: 2026-06-10（ワード検索が完成＝項目35。右コラム／ハンバーガー内の検索枠から `/search` で公開記事を検索できる。SAKIさんと対話しながら見た目調整＝枠線囲みのフォーム・アキの統一・件数表示の文字サイズ。あわせてブラウザタブのタイトル二重表示を修正、デザインポリシー「アキ（余白）は揃える」を仕様書§14-Aに明文化。前回は連載別一覧ページ `/series/[slug]` 新設＝項目34。切替時期は7月下旬 or 8月末を検討中）
